    ...
    action :manage do
      converge_by('configure elasticsearch instance') do
        ...
        if new_resource.dir && !new_resource.path_conf
          new_resource.path_conf "#{new_resource.dir}/etc/elasticsearch"
        end
        if new_resource.path_conf && new_resource.default_configuration['path.conf'].nil?
          new_resource.default_configuration['path.conf'] = new_resource.path_conf
        end

        if new_resource.dir && !new_resource.path_data
          new_resource.path_data "#{new_resource.dir}/var/data/elasticsearch"
        end
        if new_resource.path_data && new_resource.default_configuration['path.data'].nil?
          new_resource.default_configuration['path.data'] = new_resource.path_data
        end

        if new_resource.dir && !new_resource.path_logs
          new_resource.path_logs "#{new_resource.dir}/var/log/elasticsearch"
        end
        if new_resource.path_logs && new_resource.default_configuration['path.logs'].nil?
          new_resource.default_configuration['path.logs'] = new_resource.path_logs
        end
